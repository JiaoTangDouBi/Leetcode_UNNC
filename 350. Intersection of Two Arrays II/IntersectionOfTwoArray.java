public class Solution {
    public int[] intersect(int[] nums1, int[] nums2) {
        ...
        ArrayList<Integer> nums1_help = new ArrayList<Integer>();
        for (int i = 0; i < nums1.length; i++) {
        	nums1_help.add(nums1[i]);
		}
		
        ArrayList<Integer> intersection = new ArrayList<Integer>();
        for (int i = 0; i < nums2.length; i++) {
			for (int j = 0; j < nums1_help.size(); j++) {
				if (nums2[i]<nums1_help.get(j)) {
					break;
				}
				else if (nums1_help.get(j)==nums2[i]) {
					intersection.add(nums1_help.get(j));
					nums1_help.remove(j);
					break;
				}
			}
